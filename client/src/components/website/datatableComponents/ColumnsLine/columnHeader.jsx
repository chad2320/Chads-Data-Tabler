...
const ColumnHeader = ({ item }) => {
    ...
    function removeColumn() { 
        ...
        if(sort1.path === path){ //If column is sort 1
            if(sort2){ //If there is a second sort. Move it to 1.
                dispatch(modifyControls({key:'sort1',value: sort2}))
                dispatch(modifyControls({key:'sort2',value: false}))
            } else { //If column is sort 2
                dispatch(modifyControls({key:'sort1',value:false}))
            }
        } else if(sort2.path === path){ //If we are deleting second sort
            dispatch(modifyControls({key:'sort2',value:false}))
        }
        dispatch(addColumns(columns.filter(x => x.title !== title)))
        dispatch(modifySingleFilter({
    ...
    function handleChange() {
        if (sort1 === false) {
            dispatch(modifyControls({key:'sort1',value:{ name: title, path: path, val: -1 }}))
            ...
            dispatch(modifyControls({key:'sort2',value:{ name: title, path: path, val: -1 }}))
