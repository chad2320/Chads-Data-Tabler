...
import BooleanCheckmark from "../components/boolean";
...
const FilterBoxesDisplay = () => {
  ...
  useEffect(() => {
    // Check what type of filters we have
    ...
    let tempDropdown = autoCompleteValue.filter(
      (obj) => obj.type === "dropdown"
    );
    if (tempDropdown.length > 0) {
      setHaveDropdown(true);
    } else {
      setHaveDropdown(false);
    }
    let tempBoolean = autoCompleteValue.filter((obj) => obj.type === "boolean");
    if (tempBoolean.length > 0) {
      setHaveBoolean(true);
    } else {
      setHaveBoolean(false);
    }
  }, [autoCompleteValue]);
      ...
      <Box display="flex" flexDirection="column">
        {haveRange ? ( //Render the range section
          <Box display="flex" flexDirection="column">
            ...
            <Grid container>
              {autoCompleteValue
                .filter((obj) => obj.type === "range")
                .map((element) => (
                  <Grid item key={element.title}>
                    <RangeSlider path={element.path} key={element.title} />
                  </Grid>
                ))}
            </Grid>
          </Box>
        ...
        {haveDropdown ? ( //Render the dropdown section
          <Box display="flex" flexDirection="column">
            ...
            <Grid container>
              {autoCompleteValue
                .filter((obj) => obj.type === "dropdown")
                .map((element) => (
                  <Grid item key={element.title}>
                    <DefaultDropdown path={element.path} key={element.title} />
                  </Grid>
                ))}
            </Grid>
          </Box>
          ...
          <Box sx={{ width: "95%" }} display="flex" flexDirection="column">
            ...
            <Grid container>
              {autoCompleteValue
                ...
                .map((element) => (
                  <Grid item key={element.title}>
                    <BooleanCheckmark path={element.path} key={element.title} />
                  </Grid>
                ))}
            </Grid>
          </Box>
